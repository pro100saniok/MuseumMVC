(function() {
	...
	$('.thumbnaill').addClass('showThumb').addClass('fancybox').attr('rel', 'group');
	...
	function checkViewport() {
		...
		var photosWidth = $('.photos').width(),
			...
			thumbnailWidth = $('a.thumbnaill:first-child').outerWidth();
	...
	function positionThumbs() {
		...
		$('a.thumbnaill.hideThumb').animate({
			'opacity': 0
		}, 500, function() {
			$(this).css({
				'display': 'none',
		...
		var containerWidth = $('.photos').width(),
			thumbRow = 0,
			thumbColumn = 0,
			thumbWidth = $('.thumbnaill img:first-child').outerWidth() + thumbsSpacing,
			thumbHeight = $('.thumbnaill img:first-child').outerHeight() + thumbsSpacing,
			maxColumns = Math.floor( containerWidth / thumbWidth );

		$('a.thumbnaill.showThumb').each(function(index){
